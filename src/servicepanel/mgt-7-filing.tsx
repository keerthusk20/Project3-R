...
// ─────────────────────────────────────────────
// DOCUMENT DATA (Tailored for MGT-7A Annual Return)
// ─────────────────────────────────────────────
const MGT7_DOCUMENTS: Record<DocKey, string> = {
    cinCertificate: "Certificate of Incorporation (COI)",
    boardResolution: "Board Resolution for Approval of Annual Return",
    directorsList: "List of Directors & Key Managerial Personnel (KMP)",
    shareholdersList: "List of Members/Shareholders as on AGM date",
    financialStatements: "Audited Financial Statements (Balance Sheet & P&L)",
    complianceCertificate: "Certificate from Company Secretary (if applicable)",
    annualReturnDraft: "Draft of Annual Return (Form MGT-7A)",
};
...
const DOCUMENT_GROUPS: DocumentGroup[] = [
    ...
    },
    {
        ...
        icon: Users,
        keys: ["directorsList", "complianceCertificate"],
    },
    {
        id: "financials",
        label: "Financial Records",
        icon: Briefcase,
        keys: ["financialStatements", "annualReturnDraft"],
    },
    {
        id: "members-data",
        label: "Members & Shareholders",
        icon: FileText,
        keys: ["shareholdersList"],
    },
];

const getIconForDoc = (key: DocKey): React.ElementType => {
    if (["cinCertificate", "boardResolution"].includes(key)) return FileText;
    if (["directorsList", "complianceCertificate"].includes(key)) return Users;
    if (["financialStatements", "annualReturnDraft"].includes(key)) return Briefcase;
    if (["shareholdersList"].includes(key)) return FileText;
    return FileText;
};
...
// MAIN COMPONENT
// ─────────────────────────────────────────────
export default function MGT7Filing() {
    ...
    const handleInitiatePayment = async () => {
        ...
        if (!location.state?.requirementsConfirmed) {
            navigate("/services/mgt-7-filing/requirements");
            return;
        }
        setIsProcessing(true);
        try {
            const servicePrice = PRICING_CONFIG["mgt7"]?.fee ?? 0;
            await initiateRazorpayPayment({
                amount: Math.round(calculateTotalWithGST(servicePrice) * 100),
                currency: "INR",
                name: "RegiBIZ - MGT-7A Filing",
                description: `Service Fee: ₹${PRICING_CONFIG["mgt7"].fee} + GST (18%): ₹${calculateGST(PRICING_CONFIG["mgt7"].fee)} = Total: ₹${calculateTotalWithGST(PRICING_CONFIG["mgt7"].fee)}`,
                prefill: {
                    name: localStorage.getItem("userName") || "",
                    email: localStorage.getItem("userEmail") || "",
                    contact: localStorage.getItem("userPhone") || "",
                },
                notes: { service: "mgt-7-filing", source: "landing-page" },
                handler: (response: RazorpaySuccessResponse) => {
                    sessionStorage.setItem("mgt7_payment_id", response.razorpay_payment_id);
                    navigate("/services/mgt-7-filing/form", {
                        state: {
    ...
    const benefits = [
        { text: "Statutory Compliance with MCA", icon: ShieldCheck },
        { text: "Update Shareholding Pattern", icon: Users },
        { text: "Avoid Heavy Penalties & Disqualification", icon: AlertCircle },
        { text: "Maintain Good Standing", icon: BadgeCheck },
    ];

    const steps = [
        { step: "01", title: "Verify Company Data", desc: "Review registered details and financial year", icon: FileText },
        { step: "02", title: "Upload Financials", desc: "Submit balance sheet and member lists", icon: Upload },
        { step: "03", title: "Digital Signing & Filing", desc: "Sign with DSC and file with ROC", icon: Landmark },
    ];

    const stats = [
        { value: "5,000+", label: "MGT-7A Filed" },
        ...
        <div className="bg-background min-h-screen text-[#e2e8f0]" style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: "0 1 auto" }}>
                        ...
                        <div>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                <span style={{ fontSize: 15, fontWeight: 800, whiteSpace: "nowrap" }}>MGT-7A (Annual Return)</span>
                                <span style={{ fontSize: 10, fontWeight: 700, color: "#fff", background: GRADIENTS.button, borderRadius: 20, padding: "3px 10px", letterSpacing: "0.05em", boxShadow: GRADIENTS.buttonGlow }}>Mandatory</span>
                            </div>
                            <p style={{ fontSize: 12, color: "#6b7280", margin: 0 }}>RegiBIZ — Annual Compliance Filing</p>
                        </div>
                    </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, marginBottom: 72, alignItems: "start" }} className="hero-grid">
                    ...
                    <div>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 20, padding: "5px 14px", marginBottom: 24, boxShadow: "0 0 20px rgba(239,68,68,0.15)" }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 700 }}>Due within 60 days of AGM</span>
                        </div>
                        <h1 style={{ fontSize: 44, fontWeight: 800, color: "#fff", lineHeight: 1.15, margin: "0 0 16px", letterSpacing: "-0.03em", textShadow: "0 2px 20px rgba(0,0,0,0.3)" }}>
                            File Your Annual<br />
                            <span style={{ display: "inline-block", textShadow: GRADIENTS.headingGlow }}>Return (MGT-7A)</span>
                        </h1>
                        <p style={{ color: "#9ca3af", fontSize: 16, lineHeight: 1.7, marginBottom: 36, maxWidth: 480 }}>Comply with Section 92 of the Companies Act. Update shareholding patterns, director details, and financial summaries with the Registrar of Companies.</p>

                        {/* Benefits */}
                        <div style={{ marginBottom: 36 }}>
                            <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16, display: "flex", alignItems: "center", gap: 6 }}><ShieldCheck size={13} color="#ef4444" /> Why File MGT-7A?</p>
                        ...
                        <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                            {[
                                { Icon: Clock, label: "48 Hr Processing", color: "#60a5fa" },
                                { Icon: CreditCard, label: "Secure Payment", color: "#60a5fa" },
                                { Icon: ShieldCheck, label: "ROC Compliant", color: "#60a5fa" }
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <p style={{ margin: 0, fontSize: 13, color: "#d1d5db", lineHeight: 1.5 }}>
                  • Filed Annual Return (Form MGT-7A) with MCA
                </p>
                <p style={{ margin: 0, fontSize: 13, color: "#d1d5db", lineHeight: 1.5 }}>
                  • Acknowledgment Receipt & Filing Summary
                </p>
              </div>
            </div>
            <div style={{ display: "flex", gap: 14, marginTop: 36 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", marginTop: 24, background: "rgba(6,182,212,0.06)", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(6,182,212,0.15)" }}>
                            {[{ v: "5K+", l: "Filed" }, { v: "48 Hrs", l: "Processing" }, { v: "99%", l: "Success" }].map(({ v, l }, i) => (
                ...
                <div style={{ marginBottom: 72 }}>
                    <div style={{ textAlign: "center", marginBottom: 44 }}>
                        ...
                        <p style={{ color: "#6b7280", fontSize: 14, maxWidth: 500, margin: "0 auto", lineHeight: 1.6 }}>Keep these documents ready for MGT-7A filing to ensure smooth annual compliance.</p>
                    </div>
                                    ...
                                    {group.keys.map((docKey) => {
                                        ...
                                        return (
                                            <div key={docKey} style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 12px", transition: "all 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(239,68,68,0.3)"; e.currentTarget.style.background = "rgba(239,68,68,0.08)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.background = "rgba(255,255,255,0.04)"; }}>
                                                <Icon size={14} color="#f97316" /><span style={{ fontSize: 13, color: "#d1d5db", fontWeight: 500 }}>{MGT7_DOCUMENTS[docKey]}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                ...
                <div style={{ marginBottom: 72, padding: "24px", background: "rgba(245, 158, 11, 0.05)", border: "1px solid rgba(245, 158, 11, 0.2)", borderRadius: 18, display: "flex", gap: 16, alignItems: "start" }}>
                    ...
                    <div>
                        <h4 style={{ fontSize: 16, fontWeight: 700, color: "#60a5fa", marginBottom: 8 }}>Important: MGT-7A Deadline</h4>
                        <p style={{ fontSize: 14, color: "#9ca3af", lineHeight: 1.6 }}> Form MGT-7A must be filed within 60 days of holding the Annual General Meeting (AGM). Late filing attracts heavy additional fees under Section 403. Ensure your financial statements are audited before filing.</p>
                    </div>
                </div>
                ...
                <div style={{ position: "relative", borderRadius: 28, overflow: "hidden", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", padding: "40px 20px", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
                    ...
                    <div style={{ position: "relative", zIndex: 1 }}>
                        <h2 style={{ fontSize: 36, fontWeight: 800, color: "#fff", marginBottom: 16, letterSpacing: "-0.025em", lineHeight: 1.2 }}>Stay Compliant with <span style={{ background: GRADIENTS.heading, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textShadow: GRADIENTS.headingGlow }}>MGT-7A</span></h2>
                        <p style={{ color: "#9ca3af", fontSize: 16, marginBottom: 42, maxWidth: 520, margin: "0 auto 42px", lineHeight: 1.7 }}>File your annual return accurately and on time to avoid penalties and maintain good standing.</p>
                        <PayBtn label="Start MGT-7A Now — ₹699 + GST" variant="primary" style={{ borderRadius: 14, padding: "14px 24px", fontSize: 14, maxWidth: "100%", boxShadow: "0 10px 40px rgba(239,68,68,0.35)" }} />
