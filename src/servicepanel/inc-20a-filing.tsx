// src/servicepanel/inc-20a-filing.tsx
...
// ─────────────────────────────────────────────
type DocKey =
  | "bankStatement" | "proofOfAddress" | "directorConsent"
  | "subscriptionMoney" | "incorporationCert" | "registeredOffice";
...
// ─────────────────────────────────────────────
// DOCUMENT DATA (Tailored for INC-20A)
// ─────────────────────────────────────────────
const INC20A_DOCUMENTS: Record<DocKey, string> = {
  bankStatement: "Bank Statement showing Subscription Money",
  proofOfAddress: "Proof of Registered Office Address",
  directorConsent: "Director's Consent & Declaration",
  subscriptionMoney: "Proof of Payment by Subscribers",
  incorporationCert: "Certificate of Incorporation (COI)",
  registeredOffice: "Utility Bill for Office Verification",
};
...
const DOCUMENT_GROUPS: DocumentGroup[] = [
  {
    ...
    icon: Banknote,
    keys: ["bankStatement", "subscriptionMoney"],
  },
  {
    id: "office-docs",
    label: "Office Verification",
    icon: Building2,
    keys: ["proofOfAddress", "registeredOffice"],
  },
  {
    id: "legal-docs",
    label: "Legal Declarations",
    icon: FileText,
    keys: ["directorConsent", "incorporationCert"],
  },
  {
    id: "compliance",
    label: "Compliance Checks",
    icon: BadgeCheck,
    keys: ["bankStatement", "directorConsent", "incorporationCert"],
  },
];

const getIconForDoc = (key: DocKey): React.ElementType => {
  if (["bankStatement", "subscriptionMoney"].includes(key)) return Banknote;
  if (["proofOfAddress", "registeredOffice"].includes(key)) return Building2;
  if (["directorConsent"].includes(key)) return PenTool;
  if (["incorporationCert"].includes(key)) return BadgeCheck;
  return FileText;
};
...
// MAIN COMPONENT
// ─────────────────────────────────────────────
export default function INC20AFiling() {
  ...
  const handleInitiatePayment = async () => {
    ...
    if (!location.state?.requirementsConfirmed) {
      navigate("/services/inc-20a-filing/requirements");
      return;
    }
    ...
    try {
      ...
      await initiateRazorpayPayment({
        ...
        currency: "INR",
        name: "RegiBIZ - INC-20A Filing",
        description: `Service Fee: ₹${PRICING_CONFIG["inc-20a"].fee} + GST (18%): ₹${calculateGST(PRICING_CONFIG["inc-20a"].fee)} = Total: ₹${calculateTotalWithGST(PRICING_CONFIG["inc-20a"].fee)}`,
        prefill: {
          name: localStorage.getItem("userName") || "",
          email: localStorage.getItem("userEmail") || "",
          contact: localStorage.getItem("userPhone") || "",
        },
        notes: { service: "inc-20a-filing", source: "landing-page" },
        handler: (response: RazorpaySuccessResponse) => {
          sessionStorage.setItem("inc20a_payment_id", response.razorpay_payment_id);
          navigate("/services/inc-20a-filing/form", {
            state: {
  ...
  const benefits = [
    { text: "Mandatory for Company Operations", icon: ShieldCheck },
    { text: "File within 180 Days of Incorporation", icon: Clock },
    { text: "Avoid Penalty & Director Disqualification", icon: AlertCircle },
    { text: "Instant MCA Acknowledgment", icon: CheckCircle2 },
  ];

  const steps = [
    { step: "01", title: "Verify Capital", desc: "Confirm subscription money receipt in bank", icon: Banknote },
    { step: "02", title: "Document Upload", desc: "Submit Bank Statement & Office Proof", icon: FileText },
    { step: "03", title: "MCA Filing", desc: "File INC-20A with Digital Signature", icon: Landmark },
  ];

  const stats = [
    { value: "5,000+", label: "INC-20A Filed" },
    { value: "24 Hrs", label: "Processing Time" },
    { value: "100%", label: "Compliance Rate" },
    { value: `₹699+GST`, label: "Transparent Pricing" },
  ];
    ...
    <div className="bg-background min-h-screen text-[#e2e8f0]" style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: "0 1 auto" }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 15, fontWeight: 800, whiteSpace: "nowrap" }}>INC-20A Filing</span>
                <span style={{ fontSize: 10, fontWeight: 700, color: "#fff", background: GRADIENTS.button, borderRadius: 20, padding: "3px 10px", letterSpacing: "0.05em", boxShadow: GRADIENTS.buttonGlow }}>Commencement</span>
              </div>
              <p style={{ fontSize: 12, color: "#6b7280", margin: 0 }}>RegiBIZ — Start your business operations legally</p>
            </div>
          </div>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 16px", position: "relative", zIndex: 1 }}>
        <div style={{ background: "rgba(6,182,212,0.1)", border: "1px solid rgba(6,182,212,0.2)", borderRadius: 16, padding: "16px 24px", marginBottom: 32, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#fff" }}> Service Fee: <span style={{ color: "#22d3ee" }}>₹699 +GST+ Govt Charges Applicable.</span></p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, marginBottom: 72, alignItems: "start" }} className="hero-grid">
          ...
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 20, padding: "5px 14px", marginBottom: 24, boxShadow: "0 0 20px rgba(239,68,68,0.15)" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700 }}>Essential for New Companies</span>
            </div>
            <h1 style={{ fontSize: "clamp(26px, 6vw, 44px)", fontWeight: 800, color: "#fff", lineHeight: 1.15, margin: "0 0 16px", letterSpacing: "-0.03em", textShadow: "0 2px 20px rgba(0,0,0,0.3)" }}>
              Declaration for<br />
              <span style={{ display: "inline-block", textShadow: GRADIENTS.headingGlow }}>Commencement of Business</span>
            </h1>
            <p style={{ color: "#9ca3af", fontSize: 16, lineHeight: 1.7, marginBottom: 36, maxWidth: 480 }}>Every company incorporated after Oct 2018 must file INC-20A before starting operations or borrowing funds. Avoid heavy penalties.</p>

            {/* Benefits */}
            <div style={{ marginBottom: 36 }}>
              <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16, display: "flex", alignItems: "center", gap: 6 }}><ShieldCheck size={13} color="#ef4444" /> Why File INC-20A?</p>
            ...
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              {[{ icon: <Clock size={13} color="#f97316" />, label: "48 Hr Processing" }, { icon: <CreditCard size={13} color="#06b6d4" />, label: "Secure Payment" }, { icon: <Lock size={13} color="#10b981" />, label: "MCA Compliant" }].map(({ icon, label }, i) => (
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: 36, backdropFilter: "blur(16px)", boxShadow: "0 8px 40px rgba(0,0,0,0.3)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <p style={{ margin: 0, fontSize: 13, color: "#d1d5db", lineHeight: 1.5 }}>
                  • INC-20A filing with  Ministry of Corporate Affairs (MCA)
                </p>
                <p style={{ margin: 0, fontSize: 13, color: "#d1d5db", lineHeight: 1.5 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", marginTop: 24, background: "rgba(6,182,212,0.06)", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(6,182,212,0.15)" }}>
              {[{ v: "10K+", l: "Filed" }, { v: "48 Hrs", l: "Processing" }, { v: "100%", l: "Compliance" }].map(({ v, l }, i) => (
        ...
        <div style={{ marginBottom: 72 }}>
          <div style={{ textAlign: "center", marginBottom: 44 }}>
            ...
            <p style={{ color: "#6b7280", fontSize: 14, maxWidth: 500, margin: "0 auto", lineHeight: 1.6 }}>Ensure you have these documents ready for INC-20A verification.</p>
          </div>
                  ...
                  {group.keys.map((docKey) => {
                    ...
                    return (
                      <div key={docKey} style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 12px", transition: "all 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(239,68,68,0.3)"; e.currentTarget.style.background = "rgba(239,68,68,0.08)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.background = "rgba(255,255,255,0.04)"; }}>
                        <Icon size={14} color="#f97316" /><span style={{ fontSize: 13, color: "#d1d5db", fontWeight: 500 }}>{INC20A_DOCUMENTS[docKey]}</span>
                      </div>
                    );
                  })}
                </div>
        ...
        <div style={{ marginBottom: 72, padding: "24px", background: "rgba(245, 158, 11, 0.05)", border: "1px solid rgba(245, 158, 11, 0.2)", borderRadius: 18, display: "flex", gap: 16, alignItems: "start" }}>
          ...
          <div>
            <h4 style={{ fontSize: 16, fontWeight: 700, color: "#60a5fa", marginBottom: 8 }}>Important: 180-Day Deadline</h4>
            <p style={{ fontSize: 14, color: "#9ca3af", lineHeight: 1.6 }}>Form INC-20A must be filed within 180 days of incorporation. Failure to do so may result in the removal of your company name from the MCA register and penalties on directors.</p>
          </div>
        </div>
        ...
        <div style={{ position: "relative", borderRadius: 28, overflow: "hidden", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", padding: "40px 20px", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#fff", marginBottom: 16, letterSpacing: "-0.025em", lineHeight: 1.2 }}>Start Your Business <span style={{ background: GRADIENTS.heading, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textShadow: GRADIENTS.headingGlow }}>Legally</span></h2>
            <p style={{ color: "#9ca3af", fontSize: 16, marginBottom: 42, maxWidth: 520, margin: "0 auto 42px", lineHeight: 1.7 }}>Complete your compliance formalities and begin operations without legal hurdles.</p>
            <PayBtn label={`Start INC-20A Now — ₹699 + GST`} variant="primary" style={{ borderRadius: 14, padding: "14px 24px", fontSize: 14, maxWidth: "100%", boxShadow: "0 10px 40px rgba(239,68,68,0.35)" }} />
            <p style={{ fontSize: 12, color: "#4b5563", marginTop: 20, fontWeight: 500 }}><span style={{ color: "#60a5fa" }}>✓</span> MCA Compliant &nbsp;•&nbsp; <span style={{ color: "#60a5fa" }}>✓</span> No hidden charges &nbsp;•&nbsp; <span style={{ color: "#60a5fa" }}>✓</span> Expert Review</p>
          </div>
        </div>
