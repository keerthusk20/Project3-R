// src/servicepanel/adt-1-filing.tsx
...
// ─────────────────────────────────────────────
// DOCUMENT DATA (Tailored for ADT-1)
// ─────────────────────────────────────────────
const ADT1_DOCUMENTS: Record<DocKey, string> = {
  boardResolution: "Board Resolution for Auditor Appointment",
  auditorConsent: "Auditor's Consent Letter (Form ADT-1)",
  companyPan: "Company PAN Card",
  coi: "Certificate of Incorporation",
  auditorCertificate: "Auditor's Certificate (Eligibility)",
  registeredOffice: "Registered Office Address Proof",
  financialStmt: "Latest Financial Statements",
...
const DOCUMENT_GROUPS: DocumentGroup[] = [
  ...
  },
  {
    ...
    icon: FileText,
    keys: ["boardResolution", "auditorConsent", "companyPan", "coi"],
  },
];

const getIconForDoc = (key: DocKey): React.ElementType => {
  if (["companyPan", "coi", "financialStmt"].includes(key)) return FileText;
  if (["boardResolution", "auditorConsent", "intimationLetter"].includes(key)) return PenTool;
  if (["auditorCertificate"].includes(key)) return BadgeCheck;
...
export default function ADT1Filing() {
  ...
  const handleInitiatePayment = async () => {
    ...
    if (!location.state?.requirementsConfirmed) {
      navigate("/services/adt-1-filing/requirements");
      return;
    }
    ...
    try {
      ...
      await initiateRazorpayPayment({
        ...
        currency: "INR",
        name: "RegiBIZ - ADT-1 Filing",
        description: `Service Fee: ₹${PRICING_CONFIG["adt-1"].fee} + GST (18%): ₹${calculateGST(PRICING_CONFIG["adt-1"].fee)} = Total: ₹${calculateTotalWithGST(PRICING_CONFIG["adt-1"].fee)}`,
        prefill: {
          name: localStorage.getItem("userName") || "",
          email: localStorage.getItem("userEmail") || "",
          contact: localStorage.getItem("userPhone") || "",
        },
        notes: { service: "adt-1-filing", source: "landing-page" },
        handler: (response: RazorpaySuccessResponse) => {
          sessionStorage.setItem("adt1_payment_id", response.razorpay_payment_id);
          navigate("/services/adt-1-filing/form", {
            state: {
  ...
  const benefits = [
    ...
    { text: "Within 30 Days of Auditor Appointment", icon: Clock },
    { text: "Expert Verification & Review", icon: BadgeCheck },
    { text: "Instant Filing Acknowledgment", icon: CheckCircle2 },
  ];

  const steps = [
    { step: "01", title: "Submit Documents", desc: "Upload Board Resolution & Auditor Consent", icon: FileText },
    { step: "02", title: "Expert Review", desc: "Our CA reviews all documents for accuracy", icon: BadgeCheck },
    { step: "03", title: "MCA Filing", desc: "File ADT-1 on MCA portal with DSC", icon: Landmark },
  ];

  const stats = [
    { value: "5,000+", label: "ADT-1 Filed" },
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, flex: 1 }}>
              <button
                ...
                onMouseEnter={(e) => { if (!isProcessing) { e.currentTarget.style.color = "#f97316"; e.currentTarget.style.borderColor = "rgba(239,68,68,0.4)"; e.currentTarget.style.background = "rgba(239,68,68,0.08)"; } }}
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 16, fontWeight: 800, }}>ADT-1 Filing</span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#fff", background: GRADIENTS.button, borderRadius: 20, padding: "2px 8px", letterSpacing: "0.05em" }}>MCA Form</span>
                </div>
                <p style={{ fontSize: 11, color: "#6b7280", margin: 0 }}>RegiBIZ — Trusted by 5,000+ companies</p>
              </div>
            </div>
            ...
            <div className="header-cta" style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 20, fontWeight: 800, color: "#e0e8f3ff" }}>₹699 + GST</div>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 16px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, marginBottom: 72, alignItems: "start" }} className="hero-grid">
          ...
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 20, padding: "5px 14px", marginBottom: 24, boxShadow: "0 0 20px rgba(239,68,68,0.15)" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700 }}>Fastest ADT-1 Filing Service</span>
            </div>
            <h1 style={{ fontSize: 44, fontWeight: 800, color: "#fff", lineHeight: 1.15, margin: "0 0 16px", letterSpacing: "-0.03em", textShadow: "0 2px 20px rgba(0,0,0,0.3)" }}>
              Intimate Auditor<br />
              <span style={{ display: "inline-block", textShadow: GRADIENTS.headingGlow }}>Appointment to MCA</span>
            </h1>
            <p style={{ color: "#9ca3af", fontSize: 16, lineHeight: 1.7, marginBottom: 36, maxWidth: 480 }}>File Form ADT-1 within 15 days of auditor appointment. Mandatory compliance for all companies under Companies Act 2013.</p>
            ...
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              {[{ icon: <Clock size={13} color="#f97316" />, label: "24 Hr Processing" }, { icon: <CreditCard size={13} color="#06b6d4" />, label: "Secure Payment" }, { icon: <Lock size={13} color="#10b981" />, label: "MCA Compliant" }].map(({ icon, label }, i) => (
          ...
          <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 24, padding: 36, backdropFilter: "blur(16px)", boxShadow: "0 8px 40px rgba(0,0,0,0.3)", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <p style={{ margin: 0, fontSize: 13, color: "#d1d5db", lineHeight: 1.5 }}>
                  • ADT-1 form filed with MCA & acknowledgment receipt
                </p>
                <p style={{ margin: 0, fontSize: 13, color: "#d1d5db", lineHeight: 1.5 }}>
                  • Digital copy of filed ADT-1 form for your records
                </p>
              </div>
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", marginTop: 24, background: "rgba(6,182,212,0.06)", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(6,182,212,0.15)" }}>
              {[{ v: "5K+", l: "ADT-1 Filed" }, { v: "24 Hrs", l: "Processing" }, { v: "100%", l: "Compliance" }].map(({ v, l }, i) => (
        ...
        <div style={{ marginBottom: 72 }}>
          <div style={{ textAlign: "center", marginBottom: 44 }}>
            ...
            <p style={{ color: "#6b7280", fontSize: 14, maxWidth: 500, margin: "0 auto", lineHeight: 1.6 }}>Keep these documents ready for smooth ADT-1 filing process.</p>
          </div>
                  ...
                  {group.keys.map((docKey) => {
                    ...
                    return (
                      <div key={docKey} style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 12px", transition: "all 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(239,68,68,0.3)"; e.currentTarget.style.background = "rgba(239,68,68,0.08)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.background = "rgba(255,255,255,0.04)"; }}>
                        <Icon size={14} color="#f97316" /><span style={{ fontSize: 13, color: "#d1d5db", fontWeight: 500 }}>{ADT1_DOCUMENTS[docKey]}</span>
                      </div>
                    );
                  })}
                </div>
        ...
        <div style={{ marginBottom: 72, padding: "24px", background: "rgba(245, 158, 11, 0.05)", border: "1px solid rgba(245, 158, 11, 0.2)", borderRadius: 18, display: "flex", gap: 16, alignItems: "start" }}>
          ...
          <div>
            <h4 style={{ fontSize: 16, fontWeight: 700, color: "#60a5fa", marginBottom: 8 }}>Important: ADT-1 Filing Deadline</h4>
            <p style={{ fontSize: 14, color: "#9ca3af", lineHeight: 1.6 }}>Form ADT-1 must be filed within 15 days of the company's AGM where the auditor is appointed. Late filing attracts additional fees and penalties under the Companies Act 2013.</p>
          </div>
        </div>
        ...
        <div style={{ position: "relative", borderRadius: 28, overflow: "hidden", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", padding: "40px 20px", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            <h2 style={{ fontSize: 36, fontWeight: 800, color: "#fff", marginBottom: 16, letterSpacing: "-0.025em", lineHeight: 1.2 }}>File ADT-1 <span style={{ background: GRADIENTS.heading, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textShadow: GRADIENTS.headingGlow }}>Today</span></h2>
            <p style={{ color: "#9ca3af", fontSize: 16, marginBottom: 42, maxWidth: 520, margin: "0 auto 42px", lineHeight: 1.7 }}>Stay compliant with MCA regulations. Fast, accurate, and hassle-free filing.</p>
            <PayBtn label={`Start ADT-1 Now — ₹699 + GST`} variant="primary" style={{ borderRadius: 14, padding: "14px 20px", fontSize: 14, maxWidth: "50%", width: "50%", boxShadow: "0 10px 40px rgba(239,68,68,0.35)" }} />
            <p style={{ fontSize: 12, color: "#4b5563", marginTop: 20, fontWeight: 500 }}><span style={{ color: "#60a5fa" }}>✓</span> MCA Compliant &nbsp;•&nbsp; <span style={{ color: "#60a5fa" }}>✓</span> Includes 18% GST &nbsp;•&nbsp; <span style={{ color: "#60a5fa" }}>✓</span> Expert Review</p>
          </div>
        </div>
