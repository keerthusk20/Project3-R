...
// ─────────────────────────────────────────────
// DOCUMENT DATA (Tailored for AOC-4)
// ─────────────────────────────────────────────
const AOC4_DOCUMENTS: Record<DocKey, string> = {
    financialStmt: "Audited Financial Statements (Stand-alone/Consolidated)",
    balanceSheet: "Balance Sheet as at end of Financial Year",
    profitLoss: "Statement of Profit & Loss Account",
    cashFlow: "Cash Flow Statement (if applicable)",
    directorsReport: "Board’s Report (Annexures included)",
    auditorReport: "Auditor’s Report (with qualifications if any)",
    boardReport: "Copy of Board Report approved by Directors",
    complianceCert: "Certificate from Auditor/CS (if required)",
};
...
const DOCUMENT_GROUPS: DocumentGroup[] = [
    {
        ...
        icon: TrendingUp,
        keys: ["balanceSheet", "profitLoss", "cashFlow"],
    },
    {
        id: "reports",
        label: "Statutory Reports",
        icon: FileText,
        keys: ["directorsReport", "boardReport"],
    },
    {
        id: "audit-docs",
        label: "Audit & Compliance",
        icon: BadgeCheck,
        keys: ["auditorReport", "complianceCert"],
    },
    {
        id: "consolidated",
        label: "Consolidated (If Applicable)",
        icon: Briefcase,
        keys: ["financialStmt"],
    },
];
...
const getIconForDoc = (key: DocKey): React.ElementType => {
    if (["balanceSheet", "profitLoss", "cashFlow"].includes(key)) return PieChart;
    if (["directorsReport", "boardReport"].includes(key)) return FileText;
    if (["auditorReport", "complianceCert"].includes(key)) return BadgeCheck;
    if (["financialStmt"].includes(key)) return Briefcase;
    return FileText;
};
...
// ─────────────────────────────────────────────
const GRADIENTS = {
    heading: "linear-gradient(to right, #ef4444, #f97316)", // Red-Orange
    headingGlow: "0 0 30px rgba(249, 115, 22, 0.25)",
    button: "linear-gradient(to right, #0f766e, #075985, #1e3a8a)", // Teal-Blue
...
// MAIN COMPONENT
// ─────────────────────────────────────────────
export default function AOC4Filing() {
    ...
    const handleInitiatePayment = async () => {
        ...
        if (!location.state?.requirementsConfirmed) {
            navigate("/services/a0c4-filing/requirements");
            return;
        }
        ...
        try {
            ...
            await initiateRazorpayPayment({
                ...
                currency: "INR",
                name: "RegiBIZ - AOC-4 Filing",
                description: `Service Fee: ₹${PRICING_CONFIG["aoc4"].fee} + GST (18%): ₹${calculateGST(PRICING_CONFIG["aoc4"].fee)} = Total: ₹${calculateTotalWithGST(PRICING_CONFIG["aoc4"].fee)}`,
                prefill: {
                    name: localStorage.getItem("userName") || "",
                    email: localStorage.getItem("userEmail") || "",
                    contact: localStorage.getItem("userPhone") || "",
                },
                notes: { service: "aoc-4-filing", source: "landing-page" },
                handler: (response: RazorpaySuccessResponse) => {
                    sessionStorage.setItem("aoc4_payment_id", response.razorpay_payment_id);
                    navigate("/services/a0c4-filing/form", {
                        state: {
    ...
    const benefits = [
        { text: "Mandatory Annual Filing (Sec 137)", icon: ShieldCheck },
        { text: "File within 30 Days of AGM", icon: Clock },
        { text: "Avoid Heavy Late Fees", icon: AlertCircle },
        { text: "Expert CA Review Included", icon: BadgeCheck },
    ];

    const steps = [
        { step: "01", title: "Upload Financials", desc: "Submit Balance Sheet, P&L, and Reports", icon: FileText },
        { step: "02", title: "CA Verification", desc: "Our experts verify data for MCA compliance", icon: BadgeCheck },
        { step: "03", title: "DSC Signing & Filing", desc: "Sign with DSC and file Form AOC-4", icon: Landmark },
    ];

    const stats = [
        { value: "5,000+", label: "AOC-4 Filed" },
        ...
        <div className="bg-background min-h-screen text-[#e2e8f0]" style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0, flex: "0 1 auto" }}>
                        ...
                        <div>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                <span style={{ fontSize: 15, fontWeight: 800, whiteSpace: "nowrap" }}>AOC-4 Filing</span>
                                <span style={{ fontSize: 10, fontWeight: 700, color: "#fff", background: GRADIENTS.button, borderRadius: 20, padding: "3px 10px", letterSpacing: "0.05em", boxShadow: GRADIENTS.buttonGlow }}>Financial Stmt</span>
                            </div>
                            <p style={{ fontSize: 12, color: "#6b7280", margin: 0 }}>RegiBIZ — Annual Financial Compliance</p>
                        </div>
                    </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, marginBottom: 72, alignItems: "start" }} className="hero-grid">
                    ...
                    <div>
                        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 20, padding: "5px 14px", marginBottom: 24, boxShadow: "0 0 20px rgba(239,68,68,0.15)" }}>
                            ...
                            <span style={{ fontSize: 12, fontWeight: 700 }}>Due within 30 days of AGM</span>
                        </div>
                        <h1 style={{ fontSize: "clamp(22px, 6vw, 40px)", fontWeight: 800, color: "#fff", lineHeight: 1.15, margin: "0 0 16px", letterSpacing: "-0.03em", textShadow: "0 2px 20px rgba(0,0,0,0.3)" }}>
                            File Your Annual<br />
                            <span style={{ display: "inline-block", textShadow: GRADIENTS.headingGlow }}>Financial Statements (AOC-4)</span>
                        </h1>
                        <p style={{ color: "#9ca3af", fontSize: 14, lineHeight: 1.7, marginBottom: 36, maxWidth: 480 }}>Comply with Section 137 of the Companies Act. File your audited Balance Sheet, Profit & Loss Account, and Board's Report with the ROC.</p>

                        {/* Benefits */}
                        <div style={{ marginBottom: 36 }}>
                            <p style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 16, display: "flex", alignItems: "center", gap: 6 }}><ShieldCheck size={13} color="#ef4444" /> Why File AOC-4?</p>
                        ...
                        <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
                            {[{ icon: <Clock size={13} color="#f97316" />, label: "48 Hr Processing" }, { icon: <CreditCard size={13} color="#06b6d4" />, label: "Secure Payment" }, { icon: <ShieldCheck size={13} color="#10b981" />, label: "ROC Compliant" }].map(({ icon, label }, i) => (
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <p style={{ margin: 0, fontSize: 13, color: "#d1d5db", lineHeight: 1.5 }}>
                  • Filing of Form AOC-4 with MCA for the financial year, including all necessary attachments and schedules. 
                </p>
                <p style={{ margin: 0, fontSize: 13, color: "#d1d5db", lineHeight: 1.5 }}>
                  •  Acknowledgment receipt from MCA confirming successfull submission of the AOC-4 filing.
                </p>
              </div>
            </div>
            <div style={{ display: "flex", gap: 14, marginTop: 36 }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", marginTop: 24, background: "rgba(6,182,212,0.06)", borderRadius: 14, overflow: "hidden", border: "1px solid rgba(6,182,212,0.15)" }}>
                            {[{ v: "10K+", l: "Filed" }, { v: "48 Hrs", l: "Processing" }, { v: "100%", l: "Success" }].map(({ v, l }, i) => (
                ...
                <div style={{ marginBottom: 72 }}>
                    <div style={{ textAlign: "center", marginBottom: 44 }}>
                        ...
                        <p style={{ color: "#6b7280", fontSize: 14, maxWidth: 500, margin: "0 auto", lineHeight: 1.6 }}>Keep these documents ready for AOC-4 filing to ensure smooth annual compliance.</p>
                    </div>
                                    ...
                                    {group.keys.map((docKey) => {
                                        ...
                                        return (
                                            <div key={docKey} style={{ display: "flex", alignItems: "center", gap: 10, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 12px", transition: "all 0.2s" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(239,68,68,0.3)"; e.currentTarget.style.background = "rgba(239,68,68,0.08)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)"; e.currentTarget.style.background = "rgba(255,255,255,0.04)"; }}>
                                                <Icon size={14} color="#f97316" /><span style={{ fontSize: 13, color: "#d1d5db", fontWeight: 500 }}>{AOC4_DOCUMENTS[docKey]}</span>
                                            </div>
                                        );
                                    })}
                                </div>
                ...
                <div style={{ marginBottom: 72, padding: "24px", background: "rgba(245, 158, 11, 0.05)", border: "1px solid rgba(245, 158, 11, 0.2)", borderRadius: 18, display: "flex", gap: 16, alignItems: "start" }}>
                    ...
                    <div>
                        <h4 style={{ fontSize: 16, fontWeight: 700, color: "#60a5fa", marginBottom: 8 }}>Important: AOC-4 Deadline</h4>
                        <p style={{ fontSize: 14, color: "#9ca3af", lineHeight: 1.6 }}>Form AOC-4 must be filed within 30 days of holding the Annual General Meeting (AGM). Late filing attracts heavy additional fees under Section 403. Ensure your financial statements are audited before filing.</p>
                    </div>
                </div>
                ...
                <div style={{ position: "relative", borderRadius: 28, overflow: "hidden", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.12)", padding: "40px 20px", textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
                    ...
                    <div style={{ position: "relative", zIndex: 1 }}>
                        <h2 style={{ fontSize: 36, fontWeight: 800, color: "#fff", marginBottom: 16, letterSpacing: "-0.025em", lineHeight: 1.2 }}>Stay Compliant with <span style={{ background: GRADIENTS.heading, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", textShadow: GRADIENTS.headingGlow }}>AOC-4</span></h2>
                        <p style={{ color: "#9ca3af", fontSize: 16, marginBottom: 42, maxWidth: 520, margin: "0 auto 42px", lineHeight: 1.7 }}>File your financial statements accurately and on time to avoid penalties and maintain good standing.</p>
                        <PayBtn label="Start AOC-4 Now — ₹699 + GST" variant="primary" style={{ borderRadius: 14, padding: "14px 24px", fontSize: 14, maxWidth: "100%", boxShadow: "0 10px 40px rgba(239,68,68,0.35)" }} />
                        <p style={{ fontSize: 12, color: "#4b5563", marginTop: 20, fontWeight: 500 }}><span style={{ color: "#60a5fa" }}>✓</span> ROC Compliant &nbsp;•&nbsp; <span style={{ color: "#60a5fa" }}>✓</span> Expert Verification &nbsp;•&nbsp; <span style={{ color: "#60a5fa" }}>✓</span> Fast Processing</p>
                    </div>
                </div>
